...
class ActionLocationArenaSignature(dspy.Signature):
    ...
    available_arenas: str = dspy.InputField(
        desc="Comma-separated list of available arenas in the target sector."
    )
    ...
    try:
        ...
        for a in allowed:
            if a.lower() == cleaned_response.lower():
                return a
        if allowed:
            return allowed[0]  # Fallback to first
        return ""
...
if __name__ == "__main__":
    if not dspy.settings.lm:
        dspy.settings.configure(lm=dspy.DummyLM([{"next_area": "bedroom"}]))
    print(
