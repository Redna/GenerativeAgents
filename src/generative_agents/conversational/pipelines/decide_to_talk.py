...
llm = ChatGroq(model="llama3-8b-8192", name="decide_to_talk")
...
def decide_to_talk(context: str, current_time: str, init_agent: str, agent_with: str, last_chat_summary: str, init_agent_observation: str, agent_with_observation: str) -> bool:
    structured_llm = llm.with_structured_output(DecideToTalk)

    content = template.format(context=context, current_time=current_time, init_agent=init_agent, agent_with=agent_with, last_chat_summary=last_chat_summary, init_agent_observation=init_agent_observation, agent_with_observation=agent_with_observation)

    decide_to_talk = structured_llm.invoke([HumanMessage(content=content)])
