...
llm = ChatGroq(model="llama3-8b-8192", name="wake_up_hour")
...
def estimate_wake_up_hour(agent_name: str, agent_identity: str, agent_lifestyle: str) -> str:
    structured_llm = llm.with_structured_output(WakeUpHour)

    content = template.format(agent_name=agent_name, agent_identity=agent_identity, agent_lifestyle=agent_lifestyle)
    wake_up_hour = structured_llm.invoke([HumanMessage(content=content)])
