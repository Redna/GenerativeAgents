...
_template = """<|system|>Task: Turn the input into (subject, predicate, object).
    ...
    async def run(self): 
        ...
        for i in range(5):
            _action_event_triple_chain = LLMChain(prompt=_prompt, llm=llm, llm_kwargs={
                                                              ...
                                                              "top_k": 10,
                                                              "temperature": 0.4,
                                                              "cache_key": f"3action_event_triple_chain_{self.name}_{self.action_description}_{i}_{global_state.tick}"},
