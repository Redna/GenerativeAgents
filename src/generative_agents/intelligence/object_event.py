...
class ObjectEventSignature(dspy.Signature):
    ...
    action_description: str = dspy.InputField(
        desc="Description of the action being performed."
    )
...
def describe_object_state(
    name: str, object_name: str, object_address: str, action_description: str
    ...
    try:
        ...
        return f"{object_name} is {response.state}", (
            object_address,
            "is",
            response.state,
        )
...
if __name__ == "__main__":
    if not dspy.settings.lm:
        dspy.settings.configure(lm=dspy.DummyLM([{"state": "clean"}]))
    print(
