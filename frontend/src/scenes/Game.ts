...
import { toPixelPosition } from '../Globals';
...
class SimulationUpdateEngine {
  ...
  private applyUpdate(): void {
    ...
    for (const agent of update.agents) {
      console.log(agent);
      const agent_name = agent.name.replace(' ', '_');
...
export default class GameScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.atlas(
      'atlas',
      './assets/characters/Yuriko_Yamamoto.png',
  ...
  create() {
    ...
    window.addEventListener('agent-subscribe', (e: Event) => {
    ...
    for (const name in this.character_names) {
  ...
  public spawnSprite(name: string, cellX: number, cellY: number): Character {
    const position = toPixelPosition(cellX, cellY);
    ...
    const bubble = this.add.image(position.x + 60, position.y - 40, 'speech_bubble').setDepth(3);
    ...
    const emoji = this.add
      .text(position.x + 5, position.y - 67, this.getInitials(name) + ':🦁', textStyle)
      .setDepth(3);
    ...
    const character = new Character(
      name,
      sprite,
      new Bubble(this.getInitials(name), bubble, emoji),
      { col: cellX, row: cellY },
      'description',
      '',
      '',
  ...
  private createSpriteAnimation(name: string) {
    const left_walk_name = name + '-left-walk';
    const right_walk_name = name + '-right-walk';
    const down_walk_name = name + '-down-walk';
    const up_walk_name = name + '-up-walk';
