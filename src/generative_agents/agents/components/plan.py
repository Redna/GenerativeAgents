...
from generative_agents.common.events import (
    Action,
    Event,
    EventType,
    ObjectAction,
...
from generative_agents.intelligence.action_location_game_object import (
    action_location_game_object,
)
from generative_agents.intelligence.action_location_sector import (
...
from generative_agents.intelligence.new_decomposition_schedule import (
    create_new_decomposition_schedule,
)
from generative_agents.intelligence.object_event import describe_object_state
from generative_agents.intelligence.summarize_chat_relationship import (
    summarize_chat_relationship,
)
from generative_agents.intelligence.task_decomposition import (
    create_decomposition_schedule,
)
...
class Plan:
    ...
    def run(
        ...
        daytype: DayType,
        retrieved: dict[str, dict[str, list[PerceivedEvent]]],
        focused_event: dict[str, list[PerceivedEvent]] = None,
    ) -> dict:
        state = {
            "daytype": daytype,
            "retrieved": retrieved,
    ...
    def _react(self, state: PlanState) -> PlanState:
        ...
        if focused_event:
            ...
            log_agent(
                self.agent.name,
                f"reaction mode is {reaction_mode} with payload {payload}",
                "DEBUG",
            )
    ...
    def _long_term_planning(self, state: PlanState) -> PlanState:
        ...
        if daytype == DayType.FIRST_DAY:
            self.agent.scratch.daily_requirements = create_daily_plan(
                self.agent.scratch.identity,
        ...
        if daytype == DayType.NEW_DAY:
            ...
            log_agent(
                self.agent.name, f"new current status is {current_status}", "DEBUG"
            )
        ...
        # add up to 24 hours.
        self.agent.scratch.daily_schedule = create_hourly_schedule(
            self.agent.name,
            self.agent.scratch.identity,
        ...
        daily_plan = ",".join(
            [entry["activity"] for entry in self.agent.scratch.daily_schedule]
        )
        description = f"This is {self.agent.name}'s plan for {self.agent.scratch.time.today}: {daily_plan}."
        self.agent.scratch.daily_schedule_hourly_organzied = (
            self.agent.scratch.daily_schedule
    ...
    def _generate_daily_plan_and_current_status(self):
        retrieved_events = self._get_related_to_text(
            f"{self.agent.name}'s plan for {self.agent.scratch.time.as_string()}.",
            EventType.PLAN,
        )
        retrieved_events += self._get_related_to_text(
        ...
        return create_daily_plan_and_status(
            self.agent.name,
            self.agent.scratch.identity,
            self.agent.scratch.time.today,
    ...
    def _get_related_to_text(self, text: str, event_type: EventType = None):
        ...
        return [
            PerceivedEvent.from_db_entry(memory.text, memory.metadata)
    ...
    def _determine_action(self, state: PlanState) -> PlanState:
        ...
        if current_index == 0:
            ...
            if action_duration >= 60:
                ...
                if needs_decomposition(action_description, action_duration):
                    ...
                    ] = self._decompose_action(
                        current_index, action_description, action_duration
                    )
            ...
            if next_hour_index + 1 < len(self.agent.scratch.daily_schedule):
                ...
                if action_duration >= 60:
                    if needs_decomposition(action_description, action_duration):
                        self.agent.scratch.daily_schedule[
                            next_hour_index : next_hour_index + 1
                        ] = self._decompose_action(
            ...
            if self.agent.scratch.time.time.hour < 23:
                ...
                if action_duration >= 60:
                    if needs_decomposition(action_description, action_duration):
                        ...
                        decomposition = self._decompose_action(
                            next_hour_index, action_description, action_duration
                        )
                        self.agent.scratch.daily_schedule[
                            next_hour_index : next_hour_index + 1
        ...
        log_agent(self.agent.name, f"determined next sector: {action_sector}", "DEBUG")
        ...
        log_agent(
            self.agent.name, f"determined next pronouncio: {action_pronouncio}", "DEBUG"
        )

        action_event = self._generate_action_event_triple(action_description)
        log_agent(
            self.agent.name, f"determined next event triple: {action_event}", "DEBUG"
        )

        object_action = None
        if next_address != "<random>":
            action_object_desctiption, tripplet = (
                self._generate_action_object_description(
                    next_address, action_description
                )
            )
            log_agent(
                self.agent.name,
                f"determined next object description: {action_object_desctiption}",
                "DEBUG",
            )
            ...
            log_agent(
                self.agent.name,
                f"determined next object pronouncio: {action_object_pronunciatio}",
                "DEBUG",
            )
            subject, predicate, object_ = tripplet
            log_agent(
                self.agent.name,
                f"determined next object event triple: {subject}, {predicate}, {object_}",
                "DEBUG",
            )
    ...
    def _should_react(
        self, focused_event: dict[str, list[PerceivedEvent]], agents: dict[str, "Agent"]
    ):
        """
        ...
        def lets_talk(
            init_agent: "Agent",
            target_agent: "Agent",
            retrieved: dict[str, list[PerceivedEvent]],
        ):
            if init_agent.name == target_agent.name:
                return False

            if (
                not target_agent.scratch.action.address
            ...
            if (
                "sleeping" in target_agent.scratch.action.event.description
        ...
        def lets_react(init_agent: "Agent", target_agent: "Agent", retrieved):
            ...
            if (
                init_agent.scratch.action.address.split(":")[:-1]
                != target_agent.scratch.action.address.split(":")[:-1]
                or init_agent.scratch.tile.l2_distance(target_agent.scratch.tile) >= 4
            ):
                return ReactionMode.DO_OTHER_THINGS, None

            react_mode = self.agent._generate_decide_to_react(target_agent, retrieved)

            if react_mode == 1:
                wait_until = (
                    target_agent.scratch.action.start_time
                    + datetime.timedelta(
                        minutes=target_agent.scratch.action.duration - 1
                    )
    ...
    def _chat_react(self, agent_with: "Agent"):
        ...
        filling += [
            ConversationFilling(name=self.agent.name, utterance=utterance, end=end)
        ]
        description = self._generate_conversation_summary(filling)

        self._create_react_action(
            inserted_action=description,
            inserted_action_duration=10,
            action_address=f"<persona> {agent_with.name}",
            action_event=(self.agent.name, "chat with", agent_with.name),
        ...
        Plan(agent_with)._create_react_action(
            inserted_action=description,
            inserted_action_duration=10,
            action_address=f"<persona> {self.agent.name}",
            action_event=(agent_with.name, "chat with", self.agent.name),
            chatting_with=agent_with.name,
            chat="-",
            chatting_with_buffer={agent_with.name: 800},
            chatting_end_time=None,
        ...
        if end:
            duration_minutes = round(
                (
                    self.agent.scratch.time.time - self.agent.scratch.action.start_time
                ).total_seconds()
                / 60
            )
    ...
    def _wait_react(self, wait_time):
        ...
        inserted_action_duration = round(
            (end_time.minute + end_time.hour * 60)
            - (
                self.agent.scratch.time.time.minute
                + self.agent.scratch.time.time.hour * 60
            )
            + 1
        )

        action_address = (
            f"<waiting> {self.agent.scratch.tile.x} {self.agent.scratch.tile.y}"
        )
        ...
        self.agent._create_react_action(
            inserted_action,
    ...
    async def _generate_decide_to_talk(
        self, target_agent: "Agent", retrieved: dict[str, list[PerceivedEvent]]
    ):
        context = contextualize_event(
            agent=self.agent.name,
            identity=self.agent.scratch.identity,
            event_description=retrieved["curr_event"].description,
            events=", ".join([event.description for event in retrieved["events"]]),
            thoughts=", ".join(
                [thought.description for thought in retrieved["thoughts"]]
            ),
        )
        ...
        if last_conversation:
            last_chat_summary = (
                "last chatted at "
                + f"{last_conversation.created.strftime('%B %d, %Y, %H:%M:%S')} "
                + f"about {last_conversation.description}"
            )
        else:
    ...
    def _generate_decide_to_react(
        self, target_agent: "Agent", retrieved: dict[str, list[PerceivedEvent]]
    ):
        context = self._focused_event_to_context(retrieved)
        current_time = self.agent.scratch.time.as_string()

        return decide_to_react(
            context=context,
    ...
    def _generate_next_action_sector(self, action_description):
        ...
        current_sector_arenas = (
            self.agent.spatial_memory.get_str_accessible_sector_arenas(
        ...
        next_sector = action_sector_locations(
            agent_name=name,
            agent_home=home,
    ...
    def _generate_next_action_game_object(self, action_description, action_arena):
        arena = action_arena
        available_objects = (
            self.agent.spatial_memory.get_str_accessible_arena_game_objects(
                action_arena
            )
        )
        ...
        if not available_objects:
            ...
            available_objects = (
                self.agent.spatial_memory.get_str_accessible_arena_game_objects(arena)
            )

        game_object = action_location_game_object(
            action_description=action_description, available_objects=available_objects
        )

        return f"{arena}:{game_object}"

    def _generate_action_object_description(
        self, action_game_object, action_description
    ):

        object_name = action_game_object.split(":")[-1]
        return describe_object_state(
            name=self.agent.name,
    ...
    def _choose_retrieved(self, state: PlanState) -> PlanState:
        ...
        no_self_event_retrieved = {
            description: context
            for description, context in retrieved.items()
            if context["curr_event"].subject != self.agent.name
        }

        persona_context = [
            context
            for _, context in no_self_event_retrieved.items()
    ...
    def _generate_conversation(self, agent_with: "Agent"):
        ...
        relationship = self._generate_summarize_agent_relationship(
            agent_with, retrieved
        )

        focal_points = [
            f"{relationship}",
            f"{agent_with.name} is {agent_with.scratch.action.event.description}",
        ]
        ...
        if active_conversation:
            active_conversation_string = "\n".join(
                [
                    f"{filling.name}: {filling.utterance}"
                    for filling in active_conversation.filling
                ]
            )
        ...
        location = (
            f"{self.agent.scratch.tile.arena} in {self.agent.scratch.tile.sector}"
        )
        ...
        if (
            ...
            and (
                previous_chat := int(
                    (
                        self.agent.scratch.time.time - last_conversation.created
                    ).total_seconds()
                    / 60
                )
            )
            ...
            past_context = (
                f"{str(previous_chat)} minutes ago, {self.agent.name} and "
                f"{agent_with.name} were already {last_conversation.description}. "
    ...
    def _generate_summarize_agent_relationship(
        self, agent_with: "Agent", retrieved: list[PerceivedEvent]
    ):
        """
        ...
        return summarize_chat_relationship(
            agent=self.agent.name, agent_with=agent_with.name, statements=statements
        )

    def _generate_conversation_summary(
        self, conversation_filling: list[ConversationFilling]
    ):
    ...
    def _create_react_action(
        ...
        event_poignancy = self._rate_perception_poignancy(
            EventType.CHAT, inserted_action
        )
        ...
        if chatting_with_buffer:
            self.agent.scratch.chatting_with_buffer = {
                **self.agent.scratch.chatting_with_buffer,
                **chatting_with_buffer,
            }
    ...
    def _update_schedule(self, inserted_action, inserted_action_duration):
        ...
        new_schedule = create_new_decomposition_schedule(
            agent=self.agent.name,
            ...
            schedule_slice=self.agent.scratch.daily_schedule_hourly_organzied[
    ...
    def _decompose_action(
        self, action_index: int, action_description: str, action_duration: int
    ):
        """
        ...
        return create_decomposition_schedule(
            name=self.agent.name,
            identity=self.agent.scratch.identity,
