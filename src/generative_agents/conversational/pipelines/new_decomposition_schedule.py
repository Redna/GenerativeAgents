...
llm = ChatGroq(model="llama3-8b-8192", name="new_decomposition_schedule")
...
def create_new_decomposition_schedule(agent: str, start_hour: int, end_hour: int, new_event: str, new_event_duration: int, schedule_slice: list[tuple[str, int]]) -> list[tuple[str, int]]:
    ...
    structured_llm = llm.with_structured_output(NewDecompositionSchedule)
    content = template.format(agent=agent, start_hour=start_hour_str, end_hour=end_hour_str, new_event=new_event, new_event_duration=new_event_duration, schedule_slice=schedule_slice)
