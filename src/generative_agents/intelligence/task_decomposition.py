...
class SubtaskItem(BaseModel):
    ...
    duration_minutes: int = Field(
        description="Duration in minutes (usually 5 or 10).", default=5
    )
...
class DecompositionSignature(dspy.Signature):
    ...
    subtasks: list[SubtaskItem] = dspy.OutputField(
        desc="List of subtasks. The sum of their durations should equal total_duration.",
    )


def create_decomposition_schedule(
    name: str,
    identity: str,
                ...
                [
                    {
                        "subtasks": [
                            SubtaskItem(
                                activity_name="Start coding", duration_minutes=10
                            )
                        ]
                    }
                ]
