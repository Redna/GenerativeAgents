...
class RememberSignature(dspy.Signature):
    ...
    things_i_should_remember: str = dspy.OutputField(
        desc="Brief overview of things to remember for planning."
    )
...
class ReflectionsSignature(dspy.Signature):
    ...
    reflection_of_yestereday: str = dspy.OutputField(
        desc="Brief reflection of yesterday's thoughts in third-person."
    )
...
class DailyPlanSignature(dspy.Signature):
    ...
    identity: str = dspy.InputField(desc="Identity of the agent.")
    today: str = dspy.InputField(desc="Today's date.")
    feelings_for_today: str = dspy.InputField(
        desc="Reflected feelings/status for today."
    )

    plan_in_broad_strokes: list[PlanOutline] = dspy.OutputField(
        desc="List of planned activities with hours."
    )
...
def find_things_to_remember(
    name: str, identity: str, statements: str, today: str
) -> str:
    try:
        predict = dspy.Predict(RememberSignature)
        response = predict(
            name=name, identity=identity, statements=statements, today=today
        )
...
def create_daily_plan(
    name: str, identity: str, today: str, feelings_for_today: str
) -> dict[str, str]:
    try:
...
def create_daily_plan_and_status(
    name: str,
    identity: str,
    today: str,
    yesterday: str,
    ...
    plan = " ".join(
        [
            f"{i + 1}) {activity} at {time}"
            for i, (time, activity) in enumerate(daily_plan.items())
        ]
    )
                ...
                [
                    {
                        ...
                        "plan_in_broad_strokes": [
                            PlanOutline(hour=9, description="Wake up")
                        ],
                    }
                ]
            )
