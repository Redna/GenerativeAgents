...
class ActionSectorLocations(BaseModel):
    ...
    async def run(self):
        ...
        for i in range(5):
            _action_sector_locations_chain.llm_kwargs["cache_key"] = f"2action_sector_locations_{self.agent_name}_{global_state.tick}_{i}"
            ...
            if sector in self.agent_current_sector + self.agent_home + self.agent_current_sector:
                return sector
