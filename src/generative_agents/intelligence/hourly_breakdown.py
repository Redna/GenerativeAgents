...
class HourlyScheduleSignature(dspy.Signature):
    ...
    schedule: list[HourlyScheduleItem] = dspy.OutputField(
        desc="List of hourly activities starting from wake up time."
    )


def create_hourly_schedule(
    name: str, identity: str, daily_plan: list[dict[str, str]], wake_up_hour: str
) -> list[dict[str, str]]:
    hours = [
        "12:00 AM",
        "01:00 AM",
    ...
    plan_str = "\n".join(
        [
            f"{i + 1}.) {item['activity']} at {item['time']}"
    ...
    try:
        ...
        for hour in hours:
            ...
            if hour == wake_up_hour.zfill(8) or (
                not woke_up and hour_dt.hour == wake_up_dt.hour
            ):  # simple check
                woke_up = True
                full_schedule.append(
                    {
                        "time": hour,
                        "activity": generated_map.get(
                            hour, "Wake up and get ready for the day"
                        ),
                    }
                )
            elif not woke_up:
        ...
        return [
            {"time": h, "activity": "Sleeping" if i < 7 else "Idle"}
...
if __name__ == "__main__":
        ...
        dspy.settings.configure(
            lm=dspy.DummyLM(
                [
                    {
                        "schedule": [
                            HourlyScheduleItem(time="08:00 AM", activity="Wake up")
                        ]
                    }
                ]
            )
    ...
    hourly_schedule = create_hourly_schedule(
        name="Emily Johnson",
