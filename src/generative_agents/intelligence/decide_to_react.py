...
class DecideToReactSignature(dspy.Signature):
    ...
    agent_observation: str = dspy.InputField(
        desc="What the deciding agent is currently doing."
    )
    agent_with_observation: str = dspy.InputField(
        desc="Observation of the other agent."
    )
    initial_action_description: str = dspy.InputField(
        desc="The initial action the agent was doing."
    )

    thought_process: str = dspy.OutputField(desc="The reasoning behind the decision.")
    option: int = dspy.OutputField(
        desc="The chosen option (1 for Wait, 2 for Continue)."
    )
...
if __name__ == "__main__":
    print(
        decide_to_react(
            context="You are walking through the park going to Johns Pub.",
